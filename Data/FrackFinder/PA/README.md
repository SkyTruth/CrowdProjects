...
pad was present because the pad remains as long as any well is being drilled.
...
In order to load them into a PyBossa application, they were first converted to
JSON via `skyconvert.py` and then loaded with `pybossa_tools/createTasks.py` via
commands similar to these:

>       skyconvert.py 2005-2010/Transformations_and_QAQC/Tadpole/input_tasks/Tadpole-pa_2005-2010_Tasks.csv dartforg_input.json
>
>       createTasks.py -a {APP_NAME} \
>           -c -v -n 10 \
>           -s http://crowd.skytruth.org \
>           -k {YOUR_API_KEY} \
>           -t dartfrog_input.json \
...
The public started to slow down after they completed about 90% of all allotted
tasks.  At this point the remaining tasks were exclusively completed by SkyTruth
...
groups based on confidence level: 90% to 100%, 80% to 90%, 70% to 80%, 60% to 70%,
...
uses commands similar to these:
...
NOTE: The `createTasks.py` command is appropriate only for the first county
loaded.  All subsequent loads should replace the `-c` option with `-u` in
order to update an existing application instead of creating a new one.
...
Each year's file was then converted to JSON with `skyconvert.py` an 
loaded into a PyBossa app with `createTasks`:

>       skyconvert.py {COUNTY}.geojson {COUNTY}.json
>
>       createTasks.py -a {APP_NAME} \
>           -c -v -n 10 \
>           -s http://crowd.skytruth.org \
...
Internal > 66% agreement and public > 80%???????
...
TODO: Same process as 2005-2010 but included additional permitting data through the end of 2013
