...
rougly 25% of the tasks in this bin.  All tasks with an agreement level >= 80%
were passed on to the MoorFrog task generation phase.  This report can be found
...
While assembling this report an additional analysis was performed on the data.
A random sample of 100 sites was taken from each year (2005, 2008, 2010) and
manually classified by an operator using QGIS and the same imagery displayed
in the PyBossa application.  There is a bug in QGIS's random sampling tool that
ignores any filters applied to the input datasource so the filters must be
applied and then the data must be exported to a new file, from which a random
sample can be selected.  Queries and exports were performed in QGIS. All
queries were performed on Tadpole stats layer:
...
TODO: The output from Tadpole's `Sample Data/QAQC` step was used to generate input tasks
